...
node_client.addService(pg.Execute(
...
iface1 = node_client.addInterface(
    'interface-4', pg.IPv4Address('192.168.20.2', '255.255.255.0'))

# Node emulator2
node_emulator2 = request.RawPC('emulator2')
node_emulator2.addService(pg.Execute(
    '/bin/sh', '/bin/bash /local/repository/CloudLab/emulator2.sh'))
iface2 = node_emulator2.addInterface(
    'interface-5', pg.IPv4Address('192.168.20.1', '255.255.255.0'))
iface3 = node_emulator2.addInterface(
    'interface-6', pg.IPv4Address('192.168.2.1', '255.255.255.0'))

# Node emulator1
node_emulator1 = request.RawPC('emulator1')
node_emulator1.addService(pg.Execute(
    '/bin/sh', '/usr/bin/sudo /bin/bash /local/repository/CloudLab/emulator1.sh'))
iface4 = node_emulator1.addInterface(
    'interface-1', pg.IPv4Address('192.168.10.1', '255.255.255.0'))
iface5 = node_emulator1.addInterface(
    'interface-2', pg.IPv4Address('192.168.1.1', '255.255.255.0'))
...
node_router2.addService(pg.Execute(
    '/bin/sh', '/usr/bin/sudo /bin/bash /local/repository/CloudLab/router2.sh'))
iface10 = node_router2.addInterface(
    'interface-3', pg.IPv4Address('192.168.2.2', '255.255.255.0'))
iface11 = node_router2.addInterface(
    'interface-11', pg.IPv4Address('192.168.4.2', '255.255.255.0'))
